...
const spreadMines = (board, minesAmount) => {
    ...
    while (minesPlanted < minesAmount) {
        ...
        if (!board[rowSel][columnSel].mined) {
            board[rowSel][columnSel].mined = true; // <-- Corrigido: aqui estava só verificando, não setando
            minesPlanted++;
        }
    }
};
...
const createMinedBoard = (rows, columns, minesAmount) => {
    const board = createBoard(rows, columns); // <-- Corrigido nome da função
...
const openField = (board, row, column) => {
    const field = board[row][column]; // <-- Corrigido: acesso correto com [row][column]
    if (!field.opened) {
        field.opened = true;
